...
client: TikTokLiveClient = TikTokLiveClient(unique_id=USER_NAME)

sa = gspread.service_account(filename="keys.json")
wks =  sa.open("TikTokLiveComments").worksheet("Client Data")
...
async def on_connect(event: ViewerUpdateEvent):
    ...
    wks.update('G2', event.viewer_count)
...
def sigint_handler(signal, frame):
    print ('KeyboardInterrupt is caught. Closing client.')
    client.stop()
    client._disconnect
    sys.exit(0)
